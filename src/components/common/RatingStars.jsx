...
function RatingStars({ Review_Count, Star_Size }) {
  ...
  useEffect(() => {
    ...
    if(Review_Count?.value){
      let wholeStars = Math.floor(Review_Count?.value) || 0 
      SetStarCount({
        full: wholeStars,
        half: Number.isInteger(Review_Count?.value) ? 0 : 1,
        empty: Number.isInteger(Review_Count?.value) ? 5 - wholeStars : 4 - wholeStars,
      })
    }
    else{
      let wholeStars = Math.floor(Review_Count) || 0 
      SetStarCount({
        full: wholeStars,
